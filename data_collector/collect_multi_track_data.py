#!/usr/bin/env python3
"""
F1 Data Collector - 2022 Season (Rate Limited & Robust)
...
Usage:
    python data_collector_2022_fixed.py --collect-all
    python data_collector_2022_fixed.py --circuit "Canada" --sessions "Q,R"
    python data_collector_2022_fixed.py --resume
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('f1_data_collection_2022.log'),
...
class F1DataCollector2022:
    """
    Rate-limited F1 Data Collector for 2022 season with robust error handling.
    ...
    def __init__(self, base_dir: str = 'data', timeout: int = 300, rate_limit_delay: int = 8):
        """
        Initialize the 2022 F1 Data Collector with rate limiting.
        ...
        self.progress_file = self.progress_dir / 'collection_progress_2022.json'
        self.collected_sessions = self._load_progress()
        
        # 2022 F1 Calendar - All circuits with their session types
        self.circuits_2022 = {
            ...
            'Emilia Romagna': {'sessions': ['FP1', 'Q', 'S', 'R'], 'has_sprint': True},
            ...
            'Great Britain': {'sessions': ['FP1', 'Q', 'S', 'R'], 'has_sprint': True},
            'Austria': {'sessions': ['FP1', 'Q', 'S', 'R'], 'has_sprint': True},
            'France': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            ...
            'United States': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            'Mexico': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            'Brazil': {'sessions': ['FP1', 'Q', 'S', 'R'], 'has_sprint': True},
            'Abu Dhabi': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False}
        }
        
        # Priority circuits for focused collection
        self.priority_circuits = [
            'Bahrain', 'Australia', 'Spain', 'Monaco', 'Canada', 
            'Great Britain', 'Hungary', 'Italy', 'Singapore', 'Brazil'
        ]
        ...
        self.stats = {
            ...
            'sessions_by_type': {
                ...
                'S': {'attempted': 0, 'successful': 0, 'skipped': 0},
    ...
    def _extract_session_info(self, session, circuit: str, session_type: str) -> Dict:
        ...
        return {
            ...
            'session_details': {
                ...
                'is_sprint_weekend': self.circuits_2022.get(circuit, {}).get('has_sprint', False)
            }
        }
    ...
    def _extract_sprint_data(self, session) -> Dict:
        ...
        try:
            ...
            logger.info(f"      🏃 Sprint data extracted")
    ...
    def collect_circuit_complete(self, circuit: str) -> Dict:
        ...
        logger.info(f"\n🏁 Collecting complete {circuit} Grand Prix 2022")
        
        if circuit not in self.circuits_2022:
            logger.error(f"❌ Circuit '{circuit}' not found in 2022 calendar")
            return {'error': f'Circuit {circuit} not in 2022 calendar'}
        
        circuit_info = self.circuits_2022[circuit]
        sessions = circuit_info['sessions']
        
        logger.info(f"📋 Sessions to collect: {sessions}")
        if circuit_info['has_sprint']:
            logger.info(f"🏃 Sprint weekend detected!")
    ...
    def collect_priority_circuits(self) -> Dict:
        ...
        logger.info(f"\n🎯 Priority Circuits Collection - 2022 Season")
        ...
        for i, circuit in enumerate(self.priority_circuits, 1):
            logger.info(f"\n{'='*50}")
            logger.info(f"🏁 PRIORITY {i}/{len(self.priority_circuits)}: {circuit} Grand Prix 2022")
            logger.info(f"{'='*50}")
        ...
        summary_file = self.base_dir / 'priority_circuits_2022_summary.json'
    ...
    def collect_all_circuits(self) -> Dict:
        """Collect data for all circuits in the 2022 season with rate limiting."""
        
        logger.info(f"\n🏆 Complete 2022 Season Collection")
        logger.info(f"🏁 Total circuits: {len(self.circuits_2022)}")
        logger.info(f"⏱️ Rate limiting: {self.rate_limit_delay}s between API calls")
        logger.info(f"⏰ Estimated time: ~{len(self.circuits_2022) * 5 * self.rate_limit_delay / 60:.0f} minutes")
        
        collection_summary = {
            'collection_type': 'complete_season',
            'year': self.year,
            'total_circuits': len(self.circuits_2022),
            'rate_limit_delay': self.rate_limit_delay,
            'start_time': datetime.now().isoformat(),
            'results': {}
        }
        
        for i, circuit in enumerate(self.circuits_2022.keys(), 1):
            logger.info(f"\n{'='*60}")
            logger.info(f"🏁 {i}/{len(self.circuits_2022)}: {circuit} Grand Prix 2022")
            logger.info(f"{'='*60}")
            ...
            # Progress update
            logger.info(f"📊 Overall Progress: {i}/{len(self.circuits_2022)} circuits processed")
        
        collection_summary['end_time'] = datetime.now().isoformat()
        collection_summary['final_stats'] = self.stats.copy()
        
        # Save complete season summary
        summary_file = self.base_dir / 'complete_season_2022_summary.json'
        with open(summary_file, 'w') as f:
            json.dump(collection_summary, f, indent=2, default=str)
    ...
    def resume_collection(self) -> Dict:
        ...
        incomplete_circuits = []
        for circuit, circuit_info in self.circuits_2022.items():
            sessions = circuit_info['sessions']
        ...
        # Save resume summary
        summary_file = self.base_dir / 'resume_collection_2022_summary.json'
        with open(summary_file, 'w') as f:
    ...
    def _print_collection_summary(self):
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"📊 F1 2022 DATA COLLECTION SUMMARY")
        logger.info(f"{'='*60}")
        ...
        for session_type, session_stats in self.stats['sessions_by_type'].items():
            ...
            logger.info(f"   {session_type}: ✅{successful} ⏭️{skipped} 🎯{attempted} ({rate:.1f}%)")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='F1 Data Collector - 2022 Season (Rate Limited & Robust)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python data_collector_2022_fixed.py --collect-all
  python data_collector_2022_fixed.py --priority-circuits
  python data_collector_2022_fixed.py --circuit "Canada" --sessions "Q,R"
  python data_collector_2022_fixed.py --resume
  python data_collector_2022_fixed.py --check-progress
        """
    )
    
    parser.add_argument('--collect-all', action='store_true',
                       help='Collect data for all circuits in 2022 season')
    ...
    parser.add_argument('--sessions', type=str,
                       help='Comma-separated session types (e.g., "FP1,Q,R")')
    ...
    parser.add_argument('--list-circuits', action='store_true',
                       help='List all available circuits for 2022')
    
    args = parser.parse_args()
    
    # Enhanced header
    print("""
🏎️  F1 DATA COLLECTOR - 2022 SEASON (RATE LIMITED)
════════════════════════════════════════════════════════════════
  ⚡ New Regulations Era | Rate Limit Safe | Resume Capable
════════════════════════════════════════════════════════════════
    """)
    ...
    try:
        ...
        if args.check_progress:
            ...
            total_expected = sum(len(info['sessions']) for info in collector.circuits_2022.values())
        ...
        if args.list_circuits:
            ...
            print("🏁 Available Circuits for 2022:")
            for circuit, info in collector.circuits_2022.items():
                ...
                print(f"   • {circuit} {sprint_indicator}")
        ...
        if args.resume:
            ...
            print("🏆 Starting complete 2022 season collection...")
            estimated_hours = len(collector.circuits_2022) * 5 * args.rate_limit_delay / 3600
            ...
            if circuit not in collector.circuits_2022:
                print(f"❌ Error: Circuit '{circuit}' not found in 2022 calendar")
            ...
            if args.sessions:
                ...
                available_sessions = collector.circuits_2022[circuit]['sessions']
            ...
            print()
            print("1. 🏆 Complete 2022 Season (all circuits, all sessions)")
            print("2. 🎯 Priority Circuits (10 key tracks)")
            print("3. 🔄 Resume Collection (continue from where left off)")
            print("4. 🏁 Specific Circuit")
            print("5. 📋 Check Progress")
            print("6. 📋 List Available Circuits")
            print()
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    print("\n🏁 Available Circuits:")
                    for i, circuit in enumerate(collector.circuits_2022.keys(), 1):
                        sprint_indicator = "🏃" if collector.circuits_2022[circuit]['has_sprint'] else ""
                        print(f"   {i:2d}. {circuit} {sprint_indicator}")
                    
                    circuit_choice = input("\nEnter circuit name: ").strip()
                    if circuit_choice in collector.circuits_2022:
                    ...
                    print("\n🏁 2022 F1 Calendar with Progress:")
                    for circuit, info in collector.circuits_2022.items():
                        session_id = f"{collector.year}_{circuit}"
                        collected_count = len([s for s in collector.collected_sessions.keys() if s.startswith(session_id)])
                        total_sessions = len(info['sessions'])
                        ...
                        print(f"   {status} {circuit} {sprint_indicator} ({progress})")
                    continue
