"""
Created on Sat Jun  7 09:17:53 2025

@author: sid
F1 Data Collector - 2023 Season (Rate Limited & Robust)
...
- Updated 2023 calendar with new sprint format

Usage:
    python data_collector_2023.py --collect-all
    python data_collector_2023.py --circuit "Canada" --sessions "Q,R"
    python data_collector_2023.py --resume
"""
import fastf1
import pandas as pd
import numpy as np
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('f1_data_collection_2023.log'),
...
class F1DataCollector2023:
    """
    Rate-limited F1 Data Collector for 2023 season with robust error handling.
    ...
    - Updated 2023 sprint format handling
    """
    
    def __init__(self, base_dir: str = 'data', timeout: int = 300, rate_limit_delay: int = 8):
        """
        Initialize the 2023 F1 Data Collector with rate limiting.
        ...
        self.progress_file = self.progress_dir / 'collection_progress_2023.json'
        self.collected_sessions = self._load_progress()
        
        # 2023 F1 Calendar - Updated with new sprint format and Las Vegas addition
        self.circuits_2023 = {
            ...
            'Azerbaijan': {'sessions': ['FP1', 'SQ', 'S', 'Q', 'R'], 'has_sprint': True},  # New sprint format
            'Miami': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            ...
            'Austria': {'sessions': ['FP1', 'SQ', 'S', 'Q', 'R'], 'has_sprint': True},  # New sprint format
            'Great Britain': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            'Hungary': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            'Belgium': {'sessions': ['FP1', 'SQ', 'S', 'Q', 'R'], 'has_sprint': True},  # New sprint format
            ...
            'Qatar': {'sessions': ['FP1', 'SQ', 'S', 'Q', 'R'], 'has_sprint': True},  # New sprint format
            'United States': {'sessions': ['FP1', 'SQ', 'S', 'Q', 'R'], 'has_sprint': True},  # New sprint format
            'Mexico': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},
            'Brazil': {'sessions': ['FP1', 'SQ', 'S', 'Q', 'R'], 'has_sprint': True},  # New sprint format
            'Las Vegas': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False},  # NEW CIRCUIT
            'Abu Dhabi': {'sessions': ['FP1', 'FP2', 'FP3', 'Q', 'R'], 'has_sprint': False}
        }
        
        # Priority circuits for focused collection (updated for 2023)
        self.priority_circuits = [
            'Bahrain', 'Australia', 'Azerbaijan', 'Monaco', 'Spain', 'Canada', 
        ...
        self.stats = {
            ...
            'new_sprint_format_collected': 0  # Track new format sprints
        }
        ...
        try:
            ...
            session_data = {
                ...
                'collection_metadata': {
                    ...
                    'is_new_sprint_format': session_type in ['SQ', 'S'] and circuit in ['Azerbaijan', 'Austria', 'Belgium', 'Qatar', 'United States', 'Brazil']
                },
                'status': 'success'
            }
    ...
    def _extract_session_info(self, session, circuit: str, session_type: str) -> Dict:
        ...
        return {
            ...
            'circuit_info': {
                ...
                'is_new_circuit': circuit == 'Las Vegas'  # Flag new circuits in 2023
            },
            'session_details': {
                'total_laps': getattr(session, 'total_laps', None),
                'is_sprint_weekend': self.circuits_2023.get(circuit, {}).get('has_sprint', False),
                'is_new_sprint_format': session_type in ['SQ'] or (session_type == 'S' and circuit in ['Azerbaijan', 'Austria', 'Belgium', 'Qatar', 'United States', 'Brazil'])
            }
        }
    ...
    def _extract_sprint_qualifying_data(self, session) -> Dict:
        """Extract sprint qualifying-specific data (NEW for 2023 format)."""
        ...
        try:
            if hasattr(session, 'results') and session.results is not None and not session.results.empty:
                ...
                sprint_qualifying_data['sprint_qualifying_stats'] = {
                    ...
                    'session_format': 'sprint_qualifying_2023'
                }
            
            logger.info(f"      🏃 Sprint Qualifying data extracted (2023 format)")
    ...
    def _extract_sprint_data(self, session) -> Dict:
        """Extract sprint-specific data (updated for 2023 format)."""
        sprint_data = {}
        ...
        try:
            if hasattr(session, 'results') and session.results is not None and not session.results.empty:
                ...
                sprint_data['sprint_stats'] = {
                    ...
                    'sprint_format': '2023_new_format'  # Track the new format
                }
            
            logger.info(f"      🏃 Sprint data extracted (2023 format)")
    ...
    def collect_circuit_complete(self, circuit: str) -> Dict:
        ...
        logger.info(f"\n🏁 Collecting complete {circuit} Grand Prix 2023")
        
        if circuit not in self.circuits_2023:
            logger.error(f"❌ Circuit '{circuit}' not found in 2023 calendar")
            return {'error': f'Circuit {circuit} not in 2023 calendar'}
        
        circuit_info = self.circuits_2023[circuit]
        sessions = circuit_info['sessions']
        
        logger.info(f"📋 Sessions to collect: {sessions}")
        if circuit_info['has_sprint']:
            logger.info(f"🏃 Sprint weekend detected (2023 new format)!")
        if circuit == 'Las Vegas':
            logger.info(f"🆕 New circuit in 2023!")
        ...
        results = {
            ...
            'is_new_circuit': circuit == 'Las Vegas'
        }
    ...
    def collect_priority_circuits(self) -> Dict:
        ...
        logger.info(f"\n🎯 Priority Circuits Collection - 2023 Season")
        ...
        logger.info(f"🆕 Includes Las Vegas (new circuit) and new sprint format")
        ...
        for i, circuit in enumerate(self.priority_circuits, 1):
            logger.info(f"\n{'='*50}")
            logger.info(f"🏁 PRIORITY {i}/{len(self.priority_circuits)}: {circuit} Grand Prix 2023")
            logger.info(f"{'='*50}")
        ...
        summary_file = self.base_dir / 'priority_circuits_2023_summary.json'
    ...
    def collect_all_circuits(self) -> Dict:
        """Collect data for all circuits in the 2023 season with rate limiting."""
        
        logger.info(f"\n🏆 Complete 2023 Season Collection")
        logger.info(f"🏁 Total circuits: {len(self.circuits_2023)}")
        logger.info(f"⏱️ Rate limiting: {self.rate_limit_delay}s between API calls")
        logger.info(f"⏰ Estimated time: ~{len(self.circuits_2023) * 5 * self.rate_limit_delay / 60:.0f} minutes")
        logger.info(f"🆕 New features: Las Vegas circuit, updated sprint format")
        
        collection_summary = {
            'collection_type': 'complete_season',
            'year': self.year,
            'total_circuits': len(self.circuits_2023),
            'rate_limit_delay': self.rate_limit_delay,
            'start_time': datetime.now().isoformat(),
            'results': {},
            'new_features': {
                'las_vegas_circuit': True,
                'new_sprint_format': True,
                'sprint_qualifying_sessions': True
            }
        }
        
        for i, circuit in enumerate(self.circuits_2023.keys(), 1):
            logger.info(f"\n{'='*60}")
            logger.info(f"🏁 {i}/{len(self.circuits_2023)}: {circuit} Grand Prix 2023")
            logger.info(f"{'='*60}")
            ...
            # Progress update
            logger.info(f"📊 Overall Progress: {i}/{len(self.circuits_2023)} circuits processed")
        
        collection_summary['end_time'] = datetime.now().isoformat()
        collection_summary['final_stats'] = self.stats.copy()
        
        # Save complete season summary
        summary_file = self.base_dir / 'complete_season_2023_summary.json'
        with open(summary_file, 'w') as f:
            json.dump(collection_summary, f, indent=2, default=str)
    ...
    def collect_new_features_focus(self) -> Dict:
        """NEW: Collect data focusing on 2023's new features (Las Vegas + new sprint format)."""
        
        logger.info(f"\n🆕 2023 New Features Focus Collection")
        logger.info(f"🎯 Las Vegas Grand Prix + New Sprint Format")
        
        # Circuits with new sprint format in 2023
        new_sprint_circuits = ['Azerbaijan', 'Austria', 'Belgium', 'Qatar', 'United States', 'Brazil']
        
        collection_summary = {
            'collection_type': 'new_features_focus',
            'year': self.year,
            'start_time': datetime.now().isoformat(),
            'results': {},
            'focus_areas': {
                'las_vegas_circuit': ['Las Vegas'],
                'new_sprint_format': new_sprint_circuits
            }
        }
        
        # Collect Las Vegas (new circuit)
        logger.info(f"\n🆕 NEW CIRCUIT: Las Vegas Grand Prix 2023")
        las_vegas_results = self.collect_circuit_complete('Las Vegas')
        collection_summary['results']['Las Vegas'] = las_vegas_results
        
        # Collect new sprint format circuits
        for circuit in new_sprint_circuits:
            logger.info(f"\n🏃 NEW SPRINT FORMAT: {circuit} Grand Prix 2023")
            circuit_results = self.collect_circuit_complete(circuit)
            collection_summary['results'][circuit] = circuit_results
        ...
        # Save new features summary
        summary_file = self.base_dir / 'new_features_2023_summary.json'
        with open(summary_file, 'w') as f:
            json.dump(collection_summary, f, indent=2, default=str)
    ...
    def resume_collection(self) -> Dict:
        ...
        incomplete_circuits = []
        for circuit, circuit_info in self.circuits_2023.items():
            sessions = circuit_info['sessions']
        ...
        # Save resume summary
        summary_file = self.base_dir / 'resume_collection_2023_summary.json'
        with open(summary_file, 'w') as f:
    ...
    def _print_collection_summary(self):
        ...
        logger.info(f"\n{'='*60}")
        logger.info(f"📊 F1 2023 DATA COLLECTION SUMMARY")
        logger.info(f"{'='*60}")
        ...
        logger.info(f"   🏁 New sprint format sessions: {self.stats['new_sprint_format_collected']}")
        ...
        # Session type breakdown (including new 2023 session types)
...
def main():
    """Main function with enhanced command line interface for 2023."""
    
    parser = argparse.ArgumentParser(
        description='F1 Data Collector - 2023 Season (Rate Limited & Robust)',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python data_collector_2023.py --collect-all
  python data_collector_2023.py --priority-circuits
  python data_collector_2023.py --circuit "Las Vegas" --sessions "Q,R"
  python data_collector_2023.py --new-features-focus
  python data_collector_2023.py --resume
  python data_collector_2023.py --check-progress
        """
    )
    
    parser.add_argument('--collect-all', action='store_true',
                       help='Collect data for all circuits in 2023 season')
    parser.add_argument('--priority-circuits', action='store_true',
                       help='Collect data for priority circuits only')
    parser.add_argument('--new-features-focus', action='store_true',
                       help='Focus on 2023 new features (Las Vegas + new sprint format)')
    ...
    parser.add_argument('--list-circuits', action='store_true',
                       help='List all available circuits for 2023')
    
    args = parser.parse_args()
    
    # Enhanced header for 2023
    print("""
🏎️  F1 DATA COLLECTOR - 2023 SEASON (RATE LIMITED)
════════════════════════════════════════════════════════════════
  🆕 Las Vegas Circuit | New Sprint Format | Rate Limit Safe
════════════════════════════════════════════════════════════════
    """)
    ...
    print(f"   🆕 New Features: Las Vegas circuit, Sprint Qualifying (SQ)")
    print()
    
    try:
        # Initialize collector with rate limiting
        collector = F1DataCollector2023(
        ...
        if args.check_progress:
            ...
            total_expected = sum(len(info['sessions']) for info in collector.circuits_2023.values())
        ...
        if args.list_circuits:
            # List available circuits with 2023 specific info
            print("🏁 Available Circuits for 2023:")
            for circuit, info in collector.circuits_2023.items():
                sprint_indicator = "🏃 (New Sprint)" if info['has_sprint'] else ""
                new_indicator = "🆕 (NEW)" if circuit == 'Las Vegas' else ""
                ...
                print(f"   • {circuit} {sprint_indicator} {new_indicator}")
                print(f"     Sessions: {sessions}")
                print(f"     Progress: {collected_count}/{total_sessions} collected")
            return
        
        if args.new_features_focus:
            # Focus on 2023 new features
            print("🆕 Collecting 2023 new features (Las Vegas + new sprint format)...")
            collector.collect_new_features_focus()
            ...
            print("🏆 Starting complete 2023 season collection...")
            estimated_hours = len(collector.circuits_2023) * 5 * args.rate_limit_delay / 3600
            ...
            if circuit not in collector.circuits_2023:
                print(f"❌ Error: Circuit '{circuit}' not found in 2023 calendar")
            ...
            if args.sessions:
                ...
                available_sessions = collector.circuits_2023[circuit]['sessions']
            ...
            # Interactive mode with 2023 specific options
            print("🎮 Interactive Mode - Choose collection strategy:")
            print()
            print("1. 🏆 Complete 2023 Season (all circuits, all sessions)")
            print("2. 🎯 Priority Circuits (12 key tracks including Las Vegas)")
            print("3. 🆕 New Features Focus (Las Vegas + new sprint format)")
            print("4. 🔄 Resume Collection (continue from where left off)")
            print("5. 🏁 Specific Circuit")
            print("6. 📋 Check Progress")
            print("7. 📋 List Available Circuits")
            print()
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    print("\n🏁 Available Circuits:")
                    for i, circuit in enumerate(collector.circuits_2023.keys(), 1):
                        sprint_indicator = "🏃" if collector.circuits_2023[circuit]['has_sprint'] else ""
                        new_indicator = "🆕" if circuit == 'Las Vegas' else ""
                        print(f"   {i:2d}. {circuit} {sprint_indicator} {new_indicator}")
                    
                    circuit_choice = input("\nEnter circuit name: ").strip()
                    if circuit_choice in collector.circuits_2023:
                    ...
                    print("\n🏁 2023 F1 Calendar with Progress:")
                    for circuit, info in collector.circuits_2023.items():
                        session_id = f"{collector.year}_{circuit}"
                        collected_count = len([s for s in collector.collected_sessions.keys() if s.startswith(session_id)])
                        total_sessions = len(info['sessions'])
                        ...
                        print(f"   {status} {circuit} {sprint_indicator} {new_indicator} ({progress})")
                    continue
